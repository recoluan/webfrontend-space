...
## 刷新操作对缓存的影响（了解）
三种刷新操作：
1）普通手动刷新：cmd + R，点击刷新按钮 => 启用 协商缓存，强缓存失效
2）正常操作：输入URL 刷新、跳转链接、前进后退 => 强制缓存、协商缓存有效
...
- 第三，GET 不安全，POST 相对安全
GET 将参数拼接在 URL 上，容易被骇客进行 URL 劫持，POST是基于请求主体传递的，易于防止 CSRF。
=>所以登录、注册等涉及安全性的交互操作，一般用 POST 请求。

- 第四，GET 通过 URL 传递参数有长度限制，POST 没有限制。
`浏览器对于URL的长度有最大限制`（谷歌8KB、火狐7KB、IE2KB...），GET 请求基于问号传参，拼接URL不要过长，超出部分会被浏览器截断，POST 请求基于主体传递，没有限制。
=> 向服务器发送数据较大用 POST

- 第五，GET 会产生不可控制的缓存，POST 不会
