...
```
`不能给常量重复赋值。`
Uncaught Type: Assignment to constant variable.未捕获的类型错误：分配给常量变量
...
1. isNaN()：检测当前的数字是不是无效数字
    在[JS深入系列 - 数据类型转换](https://github.com/cxh0224/blog/issues/3)会对 isNaN 有更为细致的讲解。
...
2. 访问方式的区别
    ...
    - **引用类型是按引用访问的**。因为引用类型的值是保存在堆内存中的对象，这块不同于其它语言，JS 不允许直接访问对象的内存空间。在操作对象时，实际上是操作对象的引用而不是实际的对象。
