...
面试题：==随机打乱数组==
原理：基于sort排序，`每一次随机返回一个大于0或小于等于0的值`，从而随机决定是否交换位置。
```js
...
- 2参（callback , initialValue）
	...
	`返回`：本次回调函数返回的结果会作为下一次的 pre 的初始值，没有返回的是 undefined
...
console.log(newArr);//=>[1, 2, 3, 5, 4, "a", "b"]
```
...
let ary1 = [1, 2],
    ary2 = [3, 4, 5],
    str = 'chen';
let newAry = ary1.myConcat(ary2, str);
console.log(newAry);
```
...
```js
let cartData = [{price: 20, count: 2}, {price: 15, count: 3}, {price: 8, count: 10}];
totalMoney = cartData.reduce(function (pre, cur) {
    //=>0 + 40
    //=>40 + 45
    //=>85 + 80
...
for (let key in countObj) {
    if (countObj[key] === 3) {
        intersection.push(parseFloat(key));
    }
}
...
for (let key in countObj) {
    if (countObj[key] === 1) {
        diffResult.push(parseFloat(key));
    }
}
